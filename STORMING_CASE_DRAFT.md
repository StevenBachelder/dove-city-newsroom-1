...
## The TAD design rule
Canon notes: the attack happened THE DAY BEFORE the trial begins (all in-game dates count from that), and the Herald is a combined TV + newspaper house — the wide-angle debunking footage is our own broadcast arm's, which is why Sarah's wrong-call line lands so hard. In the wider fiction, 'Tad' is not a person but the Tactical Advance Division — an organization of coerced workers, each a victim held by debt. The mini-game honors this in two ways. First, the shadow is **organizational, never personal**: the astroturf trail in case 17 ends at three letters — T.A.D. — signed by a *ring* of accounts, never a named individual. Nothing in the mini-game demonizes a person the player will later be asked to free; the enemy on this desk is the machine, which keeps the Level 3 investigator arc (and the compassion it requires) coherent. Second, Sarah's correct-call line plants the investigation seed without resolving it: *'Someone organized this. Hold that thought.'* — the sentence the player carries into Level 3.
